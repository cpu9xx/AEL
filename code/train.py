...
def train(nb_user, nb_item, nb_hidden_ls, epoches_ls, dataloader, lr, nb_mask, train_set, test_set_dict, top_k, user_index_map=None):
    # 收集数据
    epoche_dict, precision_dict, avg_precision_dict, bayesian_avg_precision_dict= {}, {}, {}, {}
    model = parentAEs(nb_item, nb_user, nb_hidden_ls).to(cfg.device)  # Move model to device
    ...
    for epoches in epoches_ls:
        epoche_list, precision_list, avg_precision_list, bayesian_avg_precision_list = [], [], [], []
        for e in range(epoches):
            ...
            if (e + 1) % 20 == 0:
                print(f'{"==" * 5} {e + 1} {"==" * 5}')
                avg_precision, _, _, _ = test(model, test_set_dict, train_set, top_k_ls=top_k, if_avg=True)
                bayesian_avg_precision, _, _, _ = test(model, test_set_dict, train_set, top_k_ls=top_k, if_avg=True, if_bayesian=True)
                precision, _, _, _ = test(model, test_set_dict, train_set, top_k_ls=top_k, if_avg=False)
                epoche_list.append(e + 1)
                precision_list.append(precision)
                avg_precision_list.append(avg_precision)
                bayesian_avg_precision_list.append(bayesian_avg_precision)
        ...
        bayesian_avg_precision_dict[model.stage] = bayesian_avg_precision_list

        model.trained()
        test_all(model, test_set_dict, train_set, top_k=top_k, user_index_map=user_index_map)
    ...
    for stage in range(model.stage+1):
        ...
        plot_precision(epoche_dict[stage], bayesian_avg_precision_dict[stage], color='green')
    plt.show()
    


def test(model, test_set_dict, train_set, top_k_ls, if_avg=False, if_bayesian=False):
    ...
    for top_k in top_k_ls:
        ...
        if if_avg:
            ...
            for stage in range(len(model.AEs)):
                ...
                if if_bayesian:
                    likelihood = 1 / mse(out, ori_input_data.detach()).item()
            ...
            if if_bayesian:
                ...
                likelihood_sum = torch.sum(likelihoods)
                out = avg_out/likelihood_sum
            else:
    ...
    print(f"################ {model.stage} AVG: {if_avg} Bayesian: {if_bayesian} ######################")
...
def test_all(model, test_set_dict, train_set, top_k=5, user_index_map=None):
    print("##"*10)
    stage = model.stage
    model.stage = 0
    test(model, test_set_dict, train_set, top_k_ls=top_k)
    model.stage = 1
    test(model, test_set_dict, train_set, top_k_ls=top_k)
    model.stage = 2
    test(model, test_set_dict, train_set, top_k_ls=top_k)
...
def bayesian_avg_test(model, test_set_dict, train_set, top_k=5):
    model.eval()
    users = list(test_set_dict.keys())
    input_data = torch.tensor(train_set[users], dtype=torch.float).to(cfg.device)
    ori_input_data = input_data
...
if __name__ == '__main__':
    ...
    # model = parentAEs(nb_item, nb_user, nb_hidden_ls).to(cfg.device)
    # model.load_state_dict(torch.load(cfg.moe.model))
    # avg_precision, _, _, _ = test(model, test_set_dict, train_set, top_k_ls=cfg.top_k, if_avg=True)
    # bayesian_avg_precision, _, _, _ = test(model, test_set_dict, train_set, top_k_ls=cfg.top_k, if_avg=True, if_bayesian=True)
    # precision, _, _, _ = test(model, test_set_dict, train_set, top_k_ls=cfg.top_k, if_avg=False)

    train(nb_user, nb_item, nb_hidden_ls, epoches_ls=cfg.epoches_ls, dataloader=dataloader, lr=cfg.lr, nb_mask=cfg.nb_mask, train_set=train_set, test_set_dict=test_set_dict, top_k=cfg.top_k, user_index_map=None)
    end_time = time.time()
